  ...
  function renderPlans(lang) {
    ...
    document.getElementById('plans-grid').innerHTML = d.cards.map((c, i) => `
      <div class="plan-card ${i === featuredIdx ? 'featured' : ''}">
        ...
        <a href="https://agreeable-moss-0a72b2c0f.7.azurestaticapps.net/iam/sign-up?plan=${encodeURIComponent(c.name)}&price=${encodeURIComponent(c.price)}" class="btn-plan ${i === featuredIdx ? 'btn-plan-primary' : 'btn-plan-outline'}">${d.btn_hire}</a>
      </div>`).join('');
  }
